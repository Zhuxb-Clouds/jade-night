...
export const Lobby: React.FC = () => {
  const { roomId, hostGame, joinGame, isConnected, isHost, error, availableRooms, refreshRooms } =
    useP2P();
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 text-white p-4">
        <div className="bg-gray-800 p-8 rounded-lg shadow-lg text-center max-w-md w-full">
          ...
          <div className="bg-gray-900 p-4 rounded border border-gray-700 mb-6">
            ...
            <p className="font-mono text-xl font-bold text-white select-all">{roomId}</p>
          </div>
      ...
      <div className="bg-gray-800 p-8 rounded-lg shadow-lg w-full max-w-md">
              ...
              <ul className="divide-y divide-gray-800">
                {availableRooms.map((room) => (
                  <li key={room.roomId}>
                    ...
                    >
                      <div>
                        <span className="font-mono text-emerald-300 block">{room.roomId}</span>
                        <span className="text-xs text-gray-500">
                          {room.playerCount}/{room.maxPlayers} players
                        </span>
                      </div>
                      <span className="text-xs bg-emerald-900 text-emerald-200 px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition">
                        Join
                      </span>
                    </button>
                  </li>
        ...
        <div className="space-y-4">
          ...
          <div className="relative flex py-2 items-center">
            ...
            <span className="flex-shrink mx-4 text-gray-500">OR Join by Room ID</span>
            <div className="flex-grow border-t border-gray-700"></div>
          </div>
        ...
        <div className="mt-6 text-xs text-gray-500 text-center">
          <p>WebSocket server running on port 9000</p>
          <p className="font-mono mt-1">pnpm run server</p>
          {(window.location.hostname === "localhost" ||
            window.location.hostname === "127.0.0.1") && (
            <p className="text-yellow-500 mt-2">
              ⚠️ Tip: For LAN play, please access via your IP address (e.g., 192.168.x.x) instead of
              localhost.
            </p>
          )}
        </div>
